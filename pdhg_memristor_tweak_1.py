...
class MemristorMatVec:
    ...
    def __init__(self, comm: MPI.Comm, root_rank: Optional[int] = None):
        ...
        # Job-aware output directory for temp vectors
        self.job_id = os.environ.get('SLURM_JOB_ID', 'local')
        self.tmp_vec_base = os.environ.get('TMP_VEC_DIR', os.path.join('tmp_vectors', self.job_id))
        self.mv_count = 0
        if self.rank == self.root:
            os.makedirs(self.tmp_vec_base, exist_ok=True)
    ...
    def matvec(self, A, x, correction: bool = False):
        ...
        # After MVM, capture MCA's y_mem_result.csv uniquely per job
        if self.rank == self.root:
            # increment counter before using
            self.mv_count += 1
            default_csv = 'y_mem_result.csv'
            if os.path.exists(default_csv):
                dst = os.path.join(self.tmp_vec_base, f'y_mem_result_{self.job_id}_{self.mv_count:08d}.csv')
                try:
                    # move (atomic if same filesystem)
                    os.replace(default_csv, dst)
                except Exception:
                    try:
                        shutil.copy2(default_csv, dst)
                    except Exception:
                        pass
        ...
        if self.rank == self.root:
            ...
            y = None
                # also capture y_benchmark_result if present
        if self.rank == self.root:
            bcsv = 'y_benchmark_result.csv'
            if os.path.exists(bcsv):
                bdst = os.path.join(self.tmp_vec_base, f'y_benchmark_result_{self.job_id}_{self.mv_count:08d}.csv')
                try:
                    os.replace(bcsv, bdst)
                except Exception:
                    try:
                        shutil.copy2(bcsv, bdst)
                    except Exception:
                        pass
