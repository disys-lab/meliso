...
RUN pip install Cython mpi4py matplotlib

ADD . /meliso

WORKDIR "/meliso"

RUN PYTHON_PATH=$PYTHON_PATH:./build LD_LIBRARY_PATH=$LD_LIBRARY_PATH:./build/ make all

ENV PYTHON_PATH=$PYTHON_PATH:./build
