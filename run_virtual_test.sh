...
declare -A MATERIALS=(
    ["Ag-aSi"]="config_files/virtualization/test/Ag-aSi/"
    ["AlOx-HfO2"]="config_files/virtualization/test/AlOx-HfO2/"
    ["EpiRAM"]="config_files/virtualization/test/EpiRAM/"
    ["TaOx-HfOx"]="config_files/virtualization/test/TaOx-HfOx/"
)
    ...
    for expid in "${EXPIDs[@]}"; do
        ...
        for iter_limit in "${ITER_LIMIT[@]}"; do
            ...
            for ((i=1; i<=REPS; i++)); do
                ...
                REPORT_PATH="reports/virtualization/test/${material}/exp${expid}_iter_${iter_limit}_rep_${i}.txt"
